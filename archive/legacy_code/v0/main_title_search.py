...
_dct = {'07201': [1, 3, 25, 27, 78, 80, 87, 89, 282, 286],
        '233': [188, 280],
        '4005': [188, 283],
        'B': [162, 274, 524],
        'CD': [188, 281],
...
def main_files_data(path: str | Path):
    ...
    for e, file in enumerate(listdir(pdf2txt.second_path)):
        ...
        #     ('custom_exclusions', ([], operator.eq, 1, False, str.isalpha), {'exceptions': title_alpha_chunks}),
        #     ('custom_exclusions', ([], operator.lt, 3, True, str.isdigit), {'exceptions': title_digit_chunks}),
        # ]
        # filtered = noise_cancellation.full_pipeline_processing(text, filters)
        # filtered = noise_cancellation.from_clear_file(text)
    ...
    debug_file(Path(join(store_path, '1.0.1.chunks_analysis.txt')),
               [_ for _ in chunk_line_num.items()])
    debug_file(Path(join(store_path, '1.0.2.chunks_analysis.txt')),
               line_chunks)
    file_write(Path(join(store_path, '1.0.2.chunks_analysis.pkl')),
               line_chunks)
    ic(ict(), len(line_chunks))
    ...
    debug_file(Path(join(store_path, '2.0.1.chunks_analysis.txt')),
               [_ for _ in constant_text_chunks.items()])
    ...
    for e, file in enumerate(title_search.file_list):
        ...
        if str(Path(file).name) == str(title_search.template_title_file):
            ...
            # 4)
            min_dif = title_search.title_template(file, constant_text_chunks, file_title_chunk_line_num)
            # if not min_dif:
            #     ic(ict(), min_dif)
            #     return False
            # ic(ict(), file, [_ for _ in min_dif.keys()], [_ for _ in min_dif.values()])
            # ic(ict(), file, {k: v for k, v in min_dif.items()})
            # ic(ict(), min_dif)
            debug_file(Path(join(store_path, '4.0.1.min_dif.txt')),
                       {k: v for k, v in min_dif.items()})
            file_write(Path(join(store_path, '4.0.1.min_dif.pkl')), min_dif)

            # store_path_file = join(store_path, Path(title_search.template_title_file).name)
            # ic(ict(), store_path_file)
            # ic(ict(), end_time())

            # 5)
            chunks_dif_dct = title_search.chunks_distances(min_dif)
            # ic(ict(), chunks_dif_dct)
            # 5.1.chunks_distances.pkl
    ...
    #
    #         # 6)
    #         indexed_template = title_search.indexed_template(chunks_dif_dct)
    #         ic(ict(), indexed_template)
    #         debug_file(Path(join(store_path, '6.0.1.indexed_template.txt')), indexed_template)
    #         # ic(ict(), end_time())
    #         # ToDo 20240624 12:42 code works well until this point
    #
    #         # 7)
    #         final_template = title_search.final_template_(indexed_template)
    #         # save_pickles(join(store_path, '5.3.0.indexed_template.pkl'), indexed_template)
    #         ic(ict(), final_template)
    #         debug_file(Path(join(store_path, '7.0.1.final_template_.txt')), final_template)
    #         file_write(Path(join(store_path, '7.0.1.final_template_.pkl')), final_template)
    #         # ic(ict(), end_time())
    #         # ToDo 20240624 12:42 code works well until this point
    #
    #     # 8)
    #     x = title_search.x(file, constant_text_chunks,
    #                         file_title_chunk_line_num,
    #                         final_template)
    #     debug_file(Path(join(store_path, '8.0.1.x.txt')), x)
    #
    #     # ic(ict(), end_time())
    #     ic(ict(), x)
    #
    #     x_list.append(x)
    #
    # for e, i in enumerate(x_list):
    #     ic(e, '-'.join(dict2list(sorted_dict_2(i, True), False)))
    # # ic(ict(), end_time())
    # # ToDo 20240624 13:29 code works well until this point
...
'''
              '2211 HELLINIKON-AU1.5/06. SUBMISSIONS/CD Stage Submissions/'
              '20231222_CD HANDOVER/VAP/Wetransfer Handover/VAP/L00-L04 CEILINGS'
                pdf2txt = PdfToText(Path(''), Path(''))

                dst_path_list = create_dst_dirs(Path(root0), pdf_files())  # Directories are created using this function.
                # Depending on their suffix, files from the main directory were placed into the folders.
                reorganize_files(root0, dst_path_list)
                # These are the new paths
                pdf2txt.first_path = Path(join(root0, 'PDF'))
                pdf2txt.second_path = Path(join(root0, 'clean_files'))
                # This function converts pdf into text
                pdf2txt.pdf_create_text_file()
'''
