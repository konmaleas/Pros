...
class TitleSearch:
    ...
    Attributes:
        ...
        template_distances (dict): Internal storage for template distance results
    """
    ...
    template_distances: dict = field(init=False, default_factory=dict)  # Internal template distances storage
    ...
    def __post_init__(self):
        ...
        # Initialize template_distances as empty dict
                                ...
                                if line_chunk == line_chunk_:
                                    ...
                                    if file_ not in constant_text_chunks.keys():
                                        ...
                                        if line_num not in constant_text_chunks[file_]:
                                            ...
                                            value_list_ = list(
                                                    v for (k, v0) in constant_text_chunks[file_].items() for v in v0 if
                                                    k == line_num)
                        ...
                        for line_num, line in enumerate(file_list):
                            ...
                            if line_chunk == title_chunk:
                                if title_chunk in split_line:
                                    ...
                                    title_line_num_chunk.setdefault(
                                            line_chunk, {}).setdefault(file, []).append(line_num)
                                    file_title_chunk_line_num.setdefault(
                                            file, {}).setdefault(title_chunk, []).append(line_num)
    ...
    def title_template(self, file: Path,
        ...
        Now stores the result internally in self.template_distances for use by chunks_distances().
        ...
        Returns:
            dict | bool: Enhanced chunk distances with bidirectional patterns, or False if failed
        """
        ...
        try:
            ...
            self.template_distances = {}  # Clear on failure
            return False
        ...
        # Store the result internally for chunks_distances to use
        ...
        for title_chunk, distances in enhanced_distances.items():
            ...
            for distance, chunks in distances.items():
                ...
                if consistency or frequency >= 2:
                    ...
                    filtered_distances[distance] = {'chunk    s': chunks,
    ...
    # 5) Fifth analysis step - REFORMED VERSION
    def chunks_distances(self, max_chunks: int = 5) -> dict:
        """
        Extract the closest chunks to each title chunk based on distance.

        REFORMED: No longer requires min_dif parameter - uses self.template_distances instead.
