...
class MultiFilePatternAnalyzer:
    ...
    def _test_anchor_viability(self, anchor_line: List[str], file_data: Dict) -> bool:
        ...
        Aggregates ALL chunks reachable from ANY position of this anchor.
        """
        anchor_positions = []
    ...
    def _validate_single_file_adaptive(self, filename: str, file_data: Dict, pass_number: int) -> Dict:
        """
        Validate a single file with adaptive offset search.
        Pass 1: Search in expanding window, learn new offsets
        Pass 2: Use all discovered offsets
        """
        ...
        for anchor in self.viable_anchors:
            ...
            if anchor_positions:
                ...
                for template_chunk, base_distance in chunk_map.items():
                    ...
                    if pass_number == 1:
                        # Pass 1: Try template offset first, then search window
                        found_value, found_offset = self._search_with_window(
                            anchor_pos, base_distance, template_chunk, filtered_lines, file_result['found_chunks']
                        )

                        if found_value:
                            found = True
                            # Learn new offset if different from template
                            if found_offset != base_distance:
                                self._store_discovered_offset(anchor_key, template_chunk, found_offset)

                    else:  # pass_number == 2
                        # Pass 2: Try all discovered offsets for this chunk
                        found_value, found_offset = self._search_with_learned_offsets(
                            anchor_pos, template_chunk, anchor_key, filtered_lines, file_result['found_chunks']
                        )
                    ...
                    anchor_result['chunk_details'][template_chunk] = {
    ...
    def print_validation_report(self, validation_results: Dict):
        ...
        if self.discovered_offsets:
            ...
            for anchor_key, chunk_offsets in self.discovered_offsets.items():
                ...
                print(f"\nAnchor: [{anchor_str}]")
                for template_chunk, offsets in chunk_offsets.items():
                    if len(offsets) > 1:  # Only show if we learned new offsets
