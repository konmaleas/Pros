...
class MultiFilePatternAnalyzer:
    ...
    def _analyze_single_file_with_anchor(self, filename: str, filtered_lines: List[List[str]],
        ...
        print(f"ANCHOR SEARCH: Looking for {anchor_line}")
        print(f"FOUND ANCHORS: {anchor_positions} (total: {len(anchor_positions)})")

        if not anchor_positions:
            print("ERROR: No anchor positions found")
        ...
        print(f"TEMPLATE FILE: {is_template}")
        ...
        if is_template:
            ...
            if not found_chunks:
                print("ERROR: No template chunks found")
                return self._create_empty_result(filename)

            print(f"TEMPLATE CHUNKS FOUND: {list(found_chunks.keys())}")
            ...
            print(f"SELECTED ANCHOR POSITION: {best_anchor_pos}")

            distance_pattern = self._generate_anchor_distance_pattern(anchor_line, best_anchor_pos, found_chunks)
            print(f"TEMPLATE PATTERN CREATED: {distance_pattern.chunk_distances}")
            ...
            print(f"VALIDATION RULES: {self._chunk_validation_rules}")
            ...
            if not hasattr(self, '_template_distance_pattern') or self._template_distance_pattern is None:
                print("ERROR: No template pattern available")
                return self._create_empty_result(filename)

            print(f"TEMPLATE PATTERN TO MATCH: {self._template_distance_pattern.chunk_distances}")
            ...
            for i, anchor_pos in enumerate(anchor_positions):
                print(f"\n--- TRYING ANCHOR #{i + 1} AT LINE {anchor_pos} ---")
                ...
                if attempted_pattern:
                    print(f"FOUND CHUNKS: {attempted_pattern.chunk_distances}")
                    print(f"CHUNK LINES: {attempted_pattern.chunk_lines}")
                else:
                    print("FAILED: No pattern generated")
                    continue

                # Check if ALL template chunks were found
                is_complete = self._validate_complete_pattern(attempted_pattern)
                print(f"COMPLETENESS CHECK: {is_complete}")

                if is_complete:
                    distance_pattern = attempted_pattern
                    print(f"SUCCESS: Using anchor at line {anchor_pos}")
                    break  # Success! Use this anchor position
                else:
                    print(f"INCOMPLETE: Missing chunks, trying next anchor")

            # If no anchor position yielded complete pattern, return empty result
            if distance_pattern is None:
                print("FAILURE: No anchor position yielded complete pattern")
                return self._create_empty_result(filename)

        print(f"FINAL RESULT: {distance_pattern.anchor_line if distance_pattern else 'None'}")
        print(f"=== END DEBUG FOR {filename} ===\n")
    ...
    Optional[DistancePattern]:
        ...
        """
        print(f"    GENERATING PATTERN: anchor at line {anchor_pos}")
        ...
        print(f"    TEMPLATE DISTANCES: {template_distances}")
        ...
        for template_chunk, distance in template_distances.items():
            ...
            print(f"    CHECKING: distance {distance} -> line {predicted_line}")
            ...
            print(f"    FOUND CHUNK: '{actual_chunk}' at line {predicted_line}")

            if actual_chunk:
                chunk_distances[actual_chunk] = distance
                chunk_lines[actual_chunk] = predicted_line
            else:
                print(f"    MISSING: No valid chunk at line {predicted_line}")

        print(f"    RESULT: {len(chunk_distances)} chunks found: {chunk_distances}")

        if chunk_distances:
            return DistancePattern(anchor_chunk=str(anchor_line), anchor_line=anchor_pos,
    ...
    def _is_template_file(self, filename: str | Path) -> bool:
        ...
        try:
            file_chunks = Path(filename).stem.split(symbol)
