...
def get_complete_analysis_multi_anchor(filepath: Path, title: str, files: List[Path], extract_and_process_func) -> Dict:
    ...
    file_write(debug_path / Path('4.1_filtered_lines.pkl'), filtered_lines)
    ...
    viable_anchors = pattern_analyzer.find_multiple_anchors(all_file_data, max_anchors=5)

    # print(f"\n{'=' * 60}")
    # print("SELECTED VIABLE ANCHORS")
    # print(f"{'=' * 60}")
    #
    ...
    #
    #     for chunk, distance in sorted(chunks.items(), key=lambda x: x[1]):
    #         sign = '+' if distance >= 0 else ''
    #         print(f"     - {chunk}: {sign}{distance} lines")
    #
    # print(f"\n{'=' * 60}")
    #
    # print(f"\nFound {len(viable_anchors)} viable anchor(s)")
    # for i, anchor in enumerate(viable_anchors, 1):
    ...
    return {**filtering_data,
            'filtered_lines': filtered_lines, 'viable_anchors': viable_anchors,
...
if __name__ == '__main__':
    ...
    debug_path = dst_exists(Path(home('~')) / Path('PycharmProjects/Pros/files/reports/claude') / Path(timestamp))

    # Title with known split-chunk scenario
    title = 'NCM-DD-B1-DR-ARC-4410-P0'
