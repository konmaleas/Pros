...
def process_case(case_path: str) -> CaseResult:
    ...
    # Use first PDF file to infer title
    ...
    try:
        ...
        if all(r.total_chunks_found == 0 for r in results.values()):
            ...
            if viable_anchors:
                ...
                ic(ict(),
                   f"Multi-anchor validation: {validation_results['successful_files']}/{validation_results['total_files']} files successful")
            else:
        ...
        return CaseResult(case_path=case_path, pdf_count=len(pdf_files), success=True, inferred_title=title,
                          template_file=template_file, total_files_analyzed=len(results),
...
if __name__ == '__main__':
    ...
    output_dir = (Path.home() / 'PycharmProjects/Pros/claude' /
                  Path(__file__).parent.name / 'batch_debug' /
                  dt2.now().strftime('%Y%m%d_%H%M%S'))
    output_dir.mkdir(parents=True, exist_ok=True)

    from modules.file_opers import file_read
    ic(file_read(input_file))
