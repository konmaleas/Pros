...
class MultiFilePatternAnalyzer:
    ...
    def _extract_title_chunks(self) -> List[str]:
        """Extract individual chunks from the title string using multi-delimiter split."""
        return split_title_chunks(self.title)
    ...
    def analyze_all_files(self, extract_and_process_func) -> Tuple[Dict[str, FileAnalysisResult], Dict]:
        ...
        for file_path in normalized_files:
            ...
            all_file_data[str(file_path)] = {'processed_lines': processed_lines, 'filtered_lines': filtered_lines,
        ...
        ic(ict(), f"STABLE ANCHOR SEARCH: Found {len(stable_anchor_lines)} stable lines")

        if not stable_anchor_lines:
            ic(ict(), "NO STABLE ANCHORS: Files may have completely different text content")
    ...
    def _normalize_file_encodings(self) -> List[Path]:
        ...
        """
        ic(ict(), "NORMALIZING FILE ENCODINGS...")

        greek_to_latin = {'Î‘': 'A', 'Î’': 'B', 'Î“': 'G', 'Î”': 'D', 'Î•': 'E', 'Î–': 'Z', 'Î—': 'H', 'Î˜': 'T',
                          'Î™': 'I', 'Îš': 'K', 'Î›': 'L', 'Îœ': 'M', 'Î': 'N', 'Îž': 'X', 'ÎŸ': 'O', 'Î ': 'P',
                          'Î¡': 'R', 'Î£': 'S', 'Î¤': 'T', 'Î¥': 'Y', 'Î¦': 'F', 'Î§': 'C', 'Î¨': 'P', 'Î©': 'O',
                          'Î±': 'a', 'Î²': 'b', 'Î³': 'g', 'Î´': 'd', 'Îµ': 'e', 'Î¶': 'z', 'Î·': 'h', 'Î¸': 't',
                          'Î¹': 'i', 'Îº': 'k', 'Î»': 'l', 'Î¼': 'm', 'Î½': 'n', 'Î¾': 'x', 'Î¿': 'o', 'Ï€': 'p',
                          'Ï': 'r', 'Ïƒ': 's', 'Ï„': 't', 'Ï…': 'y', 'Ï†': 'f', 'Ï‡': 'c', 'Ïˆ': 'p', 'Ï‰': 'o',
                          'Ï‚': 's'}
        ...
        for file_path in self.files:
            ...
            for greek_char, latin_char in greek_to_latin.items():
                if greek_char in normalized_name:
                    ...
                    ic(ict(), f"NORMALIZED: {greek_char} â†’ {latin_char} in {file_path.name}")
            ...
            if hasattr(self, 'title') and any(greek_char in self.title for greek_char in greek_to_latin.keys()):
                ...
                if original_title != self.title:
                    ic(ict(), f"TITLE NORMALIZED: '{original_title}' â†’ '{self.title}'")
                    self.title_chunks = self._extract_title_chunks()

        ic(ict(), f"NORMALIZATION COMPLETE: {len(normalized_files)} files processed")
    ...
    def save_filtered_text_to_file(self, original_file_path: str, filtered_lines: List[List[str]]) -> None:
        ...
        try:
            ...
            ic(ict(), f"Warning: Could not save filtered text for {original_path.name}: {e}")
    ...
    def find_multiple_anchors(self, all_file_data: Dict) -> List[List[str]]:
        ...
        if not coverage_options:
            ic(ict(), "WARNING: Could not achieve complete chunk coverage with available anchors")
            return []
        ...
        ic(ict(), f"Complete coverage achieved with {len(self.viable_anchors)} anchor(s)")
        ic(ict(), f"Evaluated {len(coverage_options)} coverage options, selected best by distance")
    ...
    def _validate_coverage_from_selected(self) -> bool:
        ...
        if missing:
            ic(ict(), f"DEBUG: Missing from selected anchors: {missing}")
    ...
    def validate_complete_coverage(self) -> bool:
        ...
        if missing_chunks:
            ic(ict(), f"WARNING: Missing chunk coverage: {missing_chunks}")
            return False
    ...
    def get_anchor_coverage_report(self) -> str:
        ...
        if missing:
            ...
            report_lines.append("âœ“ Complete coverage achieved!")
    ...
    def validate_anchors_on_files(self, all_file_data: Dict) -> Dict:
        ...
        if validation_results['failed_files'] == 0:
            ic(ict(), "âœ“ All files validated successfully in Pass 1")
            return validation_results

        ic(ict(), f"\n=== PASS 2: Learned Offset Validation ===")
        ic(ict(), f"Discovered {self._count_discovered_offsets()} new offset(s) from Pass 1")
        ic(ict(), f"Revalidating {validation_results['failed_files']} failed file(s)...")
    ...
    def _run_validation_pass(self, all_file_data: Dict, pass_number: int, failed_files_only: bool = False,
        ...
        validation_results = {'total_files': len(all_file_data), 'successful_files': 0 if not previous_results else
        previous_results['successful_files'], 'failed_files': 0, 'per_file_results': previous_results[
            'per_file_results'].copy() if previous_results else {}, 'anchor_success_rates': {}}
        ...
        for anchor in self.viable_anchors:
            ...
            validation_results['anchor_success_rates'][anchor_key] = {'found_count' : 0, 'chunks_matched': 0,
                                                                      'total_chunks': len(
                                                                              self.anchor_chunk_assignments.get(
                                                                                  anchor_key, {}))}
    ...
    def _validate_single_file_adaptive(self, filename: str, file_data: Dict, pass_number: int) -> Dict:
        ...
        file_result = {'filename'        : filename, 'anchor_results': {}, 'found_chunks': set(),
                       'missing_chunks'  : set(), 'all_chunks_found': False}
        ...
        for anchor in self.viable_anchors:
            ...
            anchor_result = {'found'         : len(anchor_positions) > 0, 'positions': anchor_positions,
                             'chunks_matched': 0, 'chunk_details': {}}
    ...
    def _check_line_for_chunk_all_matches(self, line_num: int, template_chunk: str, filtered_lines: List[List[str]]) -> \
    List[str]:
        """
    ...
    def _search_with_learned_offsets(self, anchor_pos: int, template_chunk: str, anchor_key: Tuple,
                                     filtered_lines: List[List[str]], already_found: set) -> Tuple[
    ...
    def _check_line_for_chunk(self, line_num: int, template_chunk: str, filtered_lines: List[List[str]],
                              already_found: set) -> Optional[str]:
        """
    ...
    def print_validation_report(self, validation_results: Dict):
        ...
        ic(ict(), f"\n{'=' * 60}")
        ic(ict(), "ANCHOR VALIDATION REPORT (Two-Pass Pattern-Based)")
        ic(ict(), f"{'=' * 60}")

        ic(ict(), f"\nOverall Statistics:")
        ic(ict(), f"  Total files tested: {validation_results['total_files']}")
        ic(ict(), f"  Successful: {validation_results['successful_files']} ({validation_results['success_rate']:.1f}%)")
        ic(ict(), f"  Failed: {validation_results['failed_files']}")
        ...
        for anchor_key, stats in validation_results['anchor_success_rates'].items():
            ...
            ic(ict(), f"\n  [{anchor_str}]")
            ic(ict(),
               f"    Found in files: {stats['found_count']}/{validation_results['total_files']} ({found_rate:.1f}%)")
            ic(ict(),
            ...
            for anchor_key, chunk_offsets in self.discovered_offsets.items():
                ...
                for template_chunk, offsets in chunk_offsets.items():
                    if len(offsets) > 1:
                        ...
                        ic(ict(), f"  {template_chunk}: {offsets_str}")
        ...
        if validation_results['failed_files'] > 0:
            ...
            for filename, result in validation_results['per_file_results'].items():
                if not result['all_chunks_found']:
                    ic(ict(), f"  - {Path(filename).name}: Missing {result['missing_chunks']}")
        ...
        for filename, file_result in validation_results['per_file_results'].items():
            ...
            for anchor_key, anchor_data in file_result['anchor_results'].items():
                ...
                if anchor_data['positions']:
                    ...
                    if chunk_distances:
                        pattern = DistancePattern(anchor_chunk=anchor_str, anchor_line=anchor_pos,
                                chunk_distances=chunk_distances, chunk_lines=chunk_lines)
                        distance_patterns.append(pattern)

            # Create FileAnalysisResult
            result = FileAnalysisResult(filename=filename, template_file=False,
                    # We don't identify template files in multi-anchor mode
                    distance_patterns=distance_patterns, anchor_chunks=anchor_chunks,
