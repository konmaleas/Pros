...
class MultiFilePatternAnalyzer:
    ...
    def _analyze_single_file_with_anchor(self, filename: str, filtered_lines: List[List[str]],
        ...
        if is_template:
            ...
            # Non-template file: use template pattern but read actual chunks at those positions
            ...
            # Generate pattern by reading actual chunks at template positions
            distance_pattern = self._generate_template_based_pattern(filename, anchor_line, best_anchor_pos,
    ...
    def _validate_chunk_against_patterns(self, chunk: str) -> bool:
        ...
        for pattern in self._chunk_validation_rules:
            if (len(chunk_clean) == pattern['length'] and
                    chunk_clean.isalpha() == pattern['is_alpha'] and
                    chunk_clean.isdigit() == pattern['is_digit']):
                return True
    ...
    def _save_filtered_text_to_file(self, original_file_path: str, filtered_lines: List[List[str]]) -> None:
        ...
        """
        # Create output filename with custom path
        original_path = Path(original_file_path)
        output_dir = Path("filtered_texts")  # Change this to your desired directory
        output_dir.mkdir(exist_ok=True)  # Create directory if it doesn't exist
        output_filename = output_dir / f"filtered_{original_path.stem}.txt"
