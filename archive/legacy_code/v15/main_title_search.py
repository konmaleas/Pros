...
def get_complete_analysis_multi_anchor(filepath: Path, title: str, files: List[Path],
    ...
    NEW: Pattern-based validation instead of literal matching.
    ...
    for file_path in files:
        ...
        # Save filtered lines for this file
        pattern_analyzer._save_filtered_text_to_file(str(file_path), file_filtered)
    ...
    # NEW: Find multiple viable anchors for complete coverage
    ...
    # NEW: Validate anchors across all files using PATTERN MATCHING
    print("\n" + "=" * 60)
    print("VALIDATING ANCHORS ACROSS ALL FILES (Pattern-Based)")
    print("=" * 60)
    ...
    # Print discovered offsets for debugging
                ...
                if anchor_data['found'] and anchor_data['chunks_matched'] > 0:
                    ...
                    for chunk, details in anchor_data['chunk_details'].items():
                        if details['found']:
                            ...
                            actual = predicted - details['predicted_line'] + predicted + found_off
    ...
    # Save discovered offsets to debug file
    ...
    # Save chunk patterns for reference
    ...
    return {**filtering_data, 'filtered_lines': filtered_lines, 'viable_anchors': viable_anchors,
            ...
            'coverage_report'                 : coverage_report, 'validation_results': validation_results}
...
if __name__ == '__main__':
    ...
    print(
        f"Successful files: {result['validation_results']['successful_files']}/{result['validation_results']['total_files']}")
