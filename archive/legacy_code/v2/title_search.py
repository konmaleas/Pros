...
class TitleSearch:
    ...
    Attributes:
        ...
        # REFORMED: Internal state management for pipeline data
        chunk_line_num (dict): Mapping of chunks to line numbers (from step 1)
        line_chunks (list): All chunks found in files (from step 1)
        constant_text_chunks (dict): Constant chunks across files (from step 2)
        file_title_chunk_line_num (dict): Title chunk positions (from step 3)
        template_distances (dict): Template distance results (from step 4)
    """
    ...
    # REFORMED: Internal pipeline state management
    chunk_line_num: dict = field(init=False, default_factory=dict)  # Step 1 output
    line_chunks: list = field(init=False, default_factory=list)  # Step 1 output
    constant_text_chunks: dict = field(init=False, default_factory=dict)  # Step 2 output
    file_title_chunk_line_num: dict = field(init=False, default_factory=dict)  # Step 3 output
    template_distances: dict = field(init=False, default_factory=dict)  # Step 4 output
    ...
    def __post_init__(self):
        ...
        5. Initializes all internal state containers
        """
        ...
        # Initialize all internal state containers
        self.chunk_line_num = {}
        self.line_chunks = []
        self.constant_text_chunks = {}
        self.file_title_chunk_line_num = {}
    ...
    # REFORMED PIPELINE METHODS

    # 1) First analysis step - REFORMED VERSION
    def line_chunks_nums(self) -> bool:
        """
        Extract and analyze all line chunks from the text files.

        REFORMED: No longer returns values - stores results in self.chunk_line_num and self.line_chunks
        ...
        4. Stores results internally for use by subsequent methods

        Returns:
            bool: True if successful, False if failed
        """
        # Clear previous results
        self.chunk_line_num.clear()
        ...
        try:
                ...
                for line_num, line in enumerate(file_read(Path(file))):
                    for e, line_chunk in enumerate(line):
                        ...
                        if line_chunk.isdigit():  # Numeric chunks
                            self.line_chunks.append(line_chunk)
                            list_in_dict(self.chunk_line_num, line_chunk, line_num)
            ...
            print(f"Error in line_chunks_nums: {e}")
            return False

    # 2) Second analysis step - REFORMED VERSION
    def chunks_analysis(self) -> bool:
        """
        Analyze chunks to find constants across files.

        REFORMED: No longer requires line_chunks parameter - uses self.line_chunks
        Stores results in self.constant_text_chunks
        ...
        Returns:
            bool: True if successful, False if failed
        """
        # Validate that previous step completed
        if not self.line_chunks:
            raise ValueError("line_chunks is empty. Call line_chunks_nums() first.")

        # Clear previous results
        self.constant_text_chunks.clear()

        try:
            line_chunk_list = []

            # Count repetitions of each chunk across all files
            chunk_repetition = sorted_counter(self.line_chunks)
                            ...
                            try:
                                for e, line_chunk_ in enumerate(line):
                                    if line_chunk == line_chunk_:
                                        ...
                                        if file_ not in self.constant_text_chunks.keys():
                                            line_chunk_list.append(line_chunk)
                                            self.constant_text_chunks.update({file_: {line_num: [line_chunk]}})
                                        else:
                                            if line_num not in self.constant_text_chunks[file_]:
                                                line_chunk_list.append(line_chunk)
                                                self.constant_text_chunks[file_].update({line_num: [line_chunk]})
                                            else:
                                                line_chunk_list.append(line_chunk)
                                                # Merge with existing chunks on same line
                                                value_list_ = list(
                                                        v for (k, v0) in self.constant_text_chunks[file_].items() for v
                                                        in v0 if k == line_num)
                                                self.constant_text_chunks[file_][line_num] = deepcopy(
            ...
            print(f"Error in chunks_analysis: {e}")
            return False

    # 3) Third analysis step - REFORMED VERSION
    def title_analysis(self) -> bool:
        """
        Analyze which title chunks appear in which files and on which lines.

        REFORMED: No longer requires line_chunks parameter - uses self.line_chunks
        Stores results in self.file_title_chunk_line_num
        ...
        # Clear previous results
        self.file_title_chunk_line_num.clear()

        try:
            ic(ict(), len(self.line_chunks))
            ...
            print(f"Error in title_analysis: {e}")
            return False

    # 4) Fourth analysis step - REFORMED VERSION
    def title_template(self, file: Path,
                       anchor_selection_strategy: str = 'cluster',
                       max_distance: int = 100,
                       min_cluster_size: int = 3) -> bool:
        """
        Create an enhanced template with bidirectional distances, smart anchor selection, and pattern filtering.

        REFORMED: No longer requires constant_text_chunks and file_title_chunk_line_num parameters
        Uses self.constant_text_chunks and self.file_title_chunk_line_num instead
        Stores results in self.template_distances
        ...
        Returns:
            bool: True if successful, False if failed
        """
        # Validate that previous steps completed
        if not self.constant_text_chunks:
            raise ValueError("constant_text_chunks is empty. Call chunks_analysis() first.")
        if not self.file_title_chunk_line_num:
            raise ValueError("file_title_chunk_line_num is empty. Call title_analysis() first.")

        # Clear previous results
        self.template_distances.clear()

        try:
            filename = Path(file).name

            try:
                line_num_chunk = self.constant_text_chunks[filename]
                title_num_chunk_ = self.file_title_chunk_line_num[filename]
            except KeyError as e:
                return False

            # 1. SMART ANCHOR SELECTION
            selected_anchors = self._select_optimal_anchors(title_num_chunk_,
            ...
            self.template_distances = filtered_distances

            return True

        except Exception as e:
            print(f"Error in title_template: {e}")
            return False
        ...
        for title_chunk, distances in enhanced_distances.items():
            ...
            for distance, chunks in distances.items():
                ...
                if consistency or frequency >= 2:
                    ...
                    filtered_distances[distance] = {'chun    ks': chunks,
    ...
    # 5) Fifth analysis step - ALREADY REFORMED
    def chunks_distances(self, max_chunks: int = 5) -> dict:
        """
        Extract the closest chunks to each title chunk based on distance.

        REFORMED: Uses self.template_distances (no external parameters needed)
        ...
        Raises:
            ValueError: If template_distances is empty (title_template must be called first)
        """
        # Validate that template_distances has been populated
        if not self.template_distances:
            raise ValueError("template_distances is empty. Call title_template() first to populate template data.")

        closest_sorted_chunks = {}
        debug_file_path = Path(join(self.pkl_path, '5.0.final_template_.txt'))

        for title_key, idx_distance_chunks in self.template_distances.items():
    ...
    # NEW: MASTER PIPELINE METHOD
    def generate_full_template(self, target_file: Path = None,
                               anchor_selection_strategy: str = 'cluster',
                               max_distance: int = 100,
        ...
        """
        Execute the complete template generation pipeline.

        This master method orchestrates the entire 5-step process:
        1. Extract line chunks and numbers
        2. Analyze constant chunks across files
        3. Analyze title chunk positions
        4. Generate template distances
        5. Extract final closest chunks

        Args:
            target_file (Path, optional): Specific file to use for template. Uses template_title_file if None.
            anchor_selection_strategy (str): Strategy for anchor selection
            max_distance (int): Maximum distance for template patterns
            min_cluster_size (int): Minimum cluster size for clustering strategy
            max_chunks (int): Maximum chunks to return in final result
            save_intermediate (bool): Whether to save intermediate results to debug files

        Returns:
            dict: Final template with closest chunks for each title chunk

        Raises:
            ValueError: If any step in the pipeline fails
        """
        ic(ict(), "Starting full template generation pipeline")

        try:
            # Determine target file
            if target_file is None:
                target_file = None
                # Find the template title file from file list
                for file in self.file_list:
                    if str(Path(file).name) == str(self.template_title_file):
                        target_file = file
                        break

                if target_file is None:
                    raise ValueError(f"Template title file {self.template_title_file} not found in file list")

            ic(ict(), f"Using target file: {target_file}")

            # Step 1: Extract line chunks and numbers
            ic(ict(), "Step 1: Extracting line chunks...")
            if not self.line_chunks_nums():
                raise ValueError("Step 1 failed: line_chunks_nums()")

            if save_intermediate:
                self.debug_file(Path(join(self.pkl_path, '1.0.1.chunks_analysis.txt')),
                           [_ for _ in self.chunk_line_num.items()])
                self.debug_file(Path(join(self.pkl_path, '1.0.2.chunks_analysis.txt')),
                           self.line_chunks)
                file_write(Path(join(self.pkl_path, '1.0.2.chunks_analysis.pkl')),
                           self.line_chunks)

            ic(ict(), f"Step 1 complete: {len(self.line_chunks)} chunks found")

            # Step 2: Analyze constant chunks
            ic(ict(), "Step 2: Analyzing constant chunks...")
            if not self.chunks_analysis():
                raise ValueError("Step 2 failed: chunks_analysis()")

            if save_intermediate:
                self.debug_file(Path(join(self.pkl_path, '2.0.1.chunks_analysis.txt')),
                           [_ for _ in self.constant_text_chunks.items()])

            ic(ict(), f"Step 2 complete: {len(self.constant_text_chunks)} files analyzed")

            # Step 3: Analyze title chunk positions
            ic(ict(), "Step 3: Analyzing title chunks...")
            if not self.title_analysis():
                raise ValueError("Step 3 failed: title_analysis()")

            if save_intermediate:
                self.debug_file(Path(join(self.pkl_path, '3.0.1.file_title_chunk_line_num.txt')),
                           [_ for _ in self.file_title_chunk_line_num.items()])
                file_write(Path(join(self.pkl_path, '3.0.1.file_title_chunk_line_num.pkl')),
                           self.file_title_chunk_line_num)

            ic(ict(), f"Step 3 complete: {len(self.file_title_chunk_line_num)} files with title chunks")

            # Step 4: Generate template distances
            ic(ict(), "Step 4: Generating template distances...")
            if not self.title_template(target_file, anchor_selection_strategy, max_distance, min_cluster_size):
                raise ValueError("Step 4 failed: title_template()")

            if save_intermediate:
                self.debug_file(Path(join(self.pkl_path, '4.0.1.template_distances.txt')),
                           {k: v for k, v in self.template_distances.items()})
                file_write(Path(join(self.pkl_path, '4.0.1.template_distances.pkl')),
                           self.template_distances)

            ic(ict(), f"Step 4 complete: {len(self.template_distances)} title chunks processed")

            # Step 5: Extract final closest chunks
            ic(ict(), "Step 5: Extracting closest chunks...")
            final_result = self.chunks_distances(max_chunks)

            if save_intermediate:
                self.debug_file(Path(join(self.pkl_path, '5.0.1.chunks_dif_dct.txt')), final_result)
                file_write(join(self.pkl_path, '5.1.0.chunks_dif_dct.pkl'), final_result)

            ic(ict(), f"Pipeline complete: {len(final_result)} title chunks with distances")

            return final_result

        except Exception as e:
            ic(ict(), f"Pipeline failed: {e}")
            raise ValueError(f"Template generation pipeline failed: {e}")

    # UTILITY METHODS FOR PIPELINE STATE
    def get_pipeline_state(self) -> dict:
        """
        Get the current state of all pipeline data.

        Returns:
            dict: Dictionary containing all internal pipeline state
        """
        return {
            'chunk_line_n       um': self.chunk_line_num.copy(),
            'line_chunks': self.line_chunks.copy(),
            'constant_text_chun     ks': self.constant_text_chunks.copy(),
            'file_title_chunk_line_num': self.file_title_chunk_line_num.copy(),
            'template_distanc       es': self.template_distances.copy()
        }

    def clear_pipeline_state(self):
        """
        Clear all internal pipeline state.
    ...
    def has_pipeline_data(self, step: int = None) -> bool:
        """
        Check if pipeline data exists for specific step(s).

        Args:
            step (int, optional): Specific step to check (1-5). If None, checks all steps.

        Returns:
            bool: True if data exists for specified step(s)
        """
        if step is None:
            # Check all steps
            return (bool(self.line_chunks) and
                    bool(self.constant_text_chunks) and
                    bool(self.file_title_chunk_line_num) and
                    bool(self.template_distances))

        elif step == 1:
            return bool(self.line_chunks) and bool(self.chunk_line_num)
        elif step == 2:
            return bool(self.constant_text_chunks)
        elif step == 3:
            return bool(self.file_title_chunk_line_num)
        elif step == 4:
            return bool(self.template_distances)
        elif step == 5:
            # Step 5 uses results from step 4
            return bool(self.template_distances)
        else:
            raise ValueError(f"Invalid step number: {step}. Must be 1-5 or None.")

    def get_step_summary(self) -> dict:
        """
        Get a summary of each pipeline step's results.

        Returns:
            dict: Summary information for each step
        """
        return {
            'step_1_line_chunks': {
                'total_chunks': len(self.line_chunks),
                'unique_chunks': len(self.chunk_line_num),
                'complet                              ed': bool(self.line_chunks)
            },
            'step_2_constant_chunks': {
                'files_processed': len(self.constant_text_chunks),
                'complet                             ed': bool(self.constant_text_chunks)
            },
            'step_3_title_analysis': {
                'files_with_titles': len(self.file_title_chunk_line_num),
                'complet                              ed': bool(self.file_title_chunk_line_num)
            },
            'step_4_template_distances': {
                'title_chunks_processed': len(self.template_distances),
                'complet                                       ed': bool(self.template_distances)
            },
            'pipeline_complete': self.has_pipeline_data()
        }

    # LEGACY COMPATIBILITY METHODS (for backward compatibility)
    def get_line_chunks_data(self) -> tuple:
        """
        Legacy method: Get line chunks data in old return format.

        Returns:
            tuple: (chunk_line_num, line_chunks) - for backward compatibility
        """
        if not self.has_pipeline_data(1):
            raise ValueError("Line chunks data not available. Call line_chunks_nums() first.")
        return self.chunk_line_num.copy(), self.line_chunks.copy()

    def get_constant_chunks_data(self) -> dict:
        """
        Legacy method: Get constant chunks data.

        Returns:
            dict: constant_text_chunks - for backward compatibility
        """
        if not self.has_pipeline_data(2):
            raise ValueError("Constant chunks data not available. Call chunks_analysis() first.")
        return self.constant_text_chunks.copy()

    def get_title_analysis_data(self) -> dict:
        """
        Legacy method: Get title analysis data.

        Returns:
            dict: file_title_chunk_line_num - for backward compatibility
        """
        if not self.has_pipeline_data(3):
            raise ValueError("Title analysis data not available. Call title_analysis() first.")
        return self.file_title_chunk_line_num.copy()

    def get_template_distances(self) -> dict:
        """
        Get the current template distances.

        Returns:
            dict: Current template distances, empty dict if not set
        """
        return self.template_distances.copy()

    def has_template_data(self) -> bool:
        """
        Check if template distances have been calculated.

        Returns:
            bool: True if template data exists, False otherwise
        """
        return bool(self.template_distances)
