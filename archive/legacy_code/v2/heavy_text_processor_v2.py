...
class FrequencyBasedFilter:
    ...
    def _is_pattern_noise(self, word: str) -> bool:
        ...
        # More conservative measurement detection
        if word.isdigit() and len(word) == 4:
            # Only flag as noise if it's in very high frequency measurement cluster
            word_freq = self.word_frequencies.get(word, 0)
            similar_4digit_freqs = [freq for w, freq in self.word_frequencies.items() if w.isdigit() and len(w) == 4]

            # Only remove if this specific word appears very frequently
            # AND there are many other high-frequency 4-digit numbers
...
# Usage function
def process_pdf_with_adaptive_filtering(text: str,
                                        title:str,
                                        title_frequency_data: dict = None) -> Tuple[List[List[str]], Dict]:
    """
    ...
    Args:
        ...
        title_frequency_data: Dict from frequency analysis function
...
#     }
#
#     path = Path('C:/PyTests/Submission/Process/2211 au/dst/07xxx/PDF/raw_files')
#     file = Path('4005-RES-VAP-DWG-233-IC-07020-0_110252345213.txt')
