...
class MultiFilePatternAnalyzer:
    ...
    def _analyze_single_file_with_anchor(self, filename: str, filtered_lines: List[List[str]],
        ...
        if is_template:
            ...
            # Non-template file: use template pattern for ALL title chunks
            if not hasattr(self, '_template_distance_pattern') or self._template_distance_pattern is None:
                return self._create_empty_result(filename)

            # Use first anchor position (they should all work similarly)
            best_anchor_pos = anchor_positions[0]

            # Generate pattern using template distances for ALL chunks
            distance_pattern = self._generate_template_based_pattern(anchor_line, best_anchor_pos, validated_positions)
        ...
        return FileAnalysisResult(filename=filename, template_file=is_template,
                                  ...
                                  total_chunks_found=sum(len(positions)
                                                         for positions in validated_positions.values()
                                                         if any(chunk in self.title_chunks for chunk in [positions])))

    def _generate_template_based_pattern(self, anchor_line: List[str], anchor_pos: int,
                                         validated_positions: Dict[str, List[int]]) -> Optional[DistancePattern]:
        """
        Generate distance pattern using template file's pattern for ALL title chunks.
        """
        if not hasattr(self, '_template_distance_pattern') or self._template_distance_pattern is None:
            return None
        ...
        for chunk in self.title_chunks:
            if chunk in template_distances:
                # Use template distance for this chunk
                template_distance = template_distances[chunk]
                predicted_line = anchor_pos + template_distance

                chunk_distances[chunk] = template_distance
                chunk_lines[chunk] = predicted_line
            else:
                # Fallback: chunk not in template (shouldn't happen)
                chunk_distances[chunk] = None
                chunk_lines[chunk] = None

        return DistancePattern(anchor_chunk=str(anchor_line), anchor_line=anchor_pos, chunk_distances=chunk_distances,
                               chunk_lines=chunk_lines)

    def _predict_all_chunk_positions(self,
                                     anchor_positions: List[int],
                                     validated_positions: Dict[str, List[int]]) -> Dict[str, List[int]]:
        """
        Predict positions for ALL title chunks, even if not found in this file.
        Use template pattern + found chunks to estimate missing ones.
        """
        # TODO: Use template file distance pattern to predict missing chunk positions
        # For now, return found chunks + predicted positions for missing ones

        result = {}
        for chunk in self.title_chunks:
            if chunk in validated_positions:
                result[chunk] = validated_positions[chunk]
            else:
                # Predict position using template pattern (implement this logic)
                predicted_pos = self._predict_chunk_position(chunk, anchor_positions[0])
                if predicted_pos is not None:
                    result[chunk] = [predicted_pos]
    ...
    def _find_chunk_in_filtered_text(self,
                                     chunk: str,
                                     filtered_lines: List[List[str]]) -> List[int]:
    ...
    def _find_optimal_anchor_position(self,
                                      anchor_positions: List[int],
                                      found_chunks: Dict[str, List[int]]) -> int:
        """
    ...
    def _generate_anchor_distance_pattern(self,
                                          anchor_line: List[str],
                                          anchor_pos: int,
